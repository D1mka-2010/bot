...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id not in user_data:
        user_data[user_id] = {
            ...
            "in_dialog": False,
            "dialog_menu_shown": False  # Флаг, показывалось ли меню диалога
        }
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(
            assistant_message,
            reply_markup=get_end_dialog_keyboard()  # Показываем меню с кнопкой завершения
        )
...
async def end_dialog(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in user_data:
        ...
        user_data[user_id]["dialog_menu_shown"] = False  # Сбрасываем флаг меню диалога
