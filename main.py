...
def confirm_phone_step1(call):
    ...
    try:
        async def send_code():
            # Используем реалистичные параметры устройства, чтобы код приходил
...
def settings_menu(call):
    ...
    markup.row(types.InlineKeyboardButton("🔙 Назад", callback_data="back_to_main"))
    send_new_message(user_id, "⚙️ <b>НАСТРОЙКИ</b>\n\nУправляйте параметрами бота:", reply_markup=markup)
...
def handle_user_commands(message):
    ...
    if message.chat.type != "private":
        ...
        return
    # Здесь можно оставить подсказку, что реальная обработка идёт через юзербота.
