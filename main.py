...
# Этот обработчик будет принимать сообщения из бизнес-чатов (Premium)
@bot.business_message_handler(func=lambda message: True)
def handle_business_messages(message):
    # Получаем бизнес-подключение
    ...
    # Проверяем, является ли отправитель владельцем аккаунта (т.е. вы)
    # Для этого используем данные из базы – получаем ваш ID из сессии
    # Но проще – проверить, есть ли у этого пользователя активная сессия
    # Или просто сохранить ваш ID в переменной. Воспользуемся ADMIN_LIST[0] как владелец.
    owner_id = ADMIN_LIST[0] if ADMIN_LIST else None
    if owner_id and user_id != owner_id:
        # Если команду написал кто-то другой – игнорируем
        return

    # Обрабатываем команды в бизнес-чатах
    if command in ['/mute', '/unmute', '/ai', '/ai_off', '/check']:
        # Пытаемся удалить команду (в бизнес-чатах это может не работать)
        try:
    ...
    # Если это не команда – обрабатываем как обычное сообщение для ИИ или тем
    # Проверяем, включен ли ИИ или темы в этом чате
    data = load_data()
    ...
    if chat_id in data.get("ai_chats", []):
        if text and owner_id:
            ...
            # Собираем историю (здесь сложно получить историю через Bot API, можно пропустить)
            history = []
            ...
            if reply:
                ...
                time.sleep(random.uniform(1.0, 3.0))  # Грубая имитация
                bot.send_message(chat_id, reply, business_connection_id=business_connection_id, parse_mode='Markdown')

# ===================== ОСТАЛЬНЫЕ ОБРАБОТЧИКИ (АДМИНКА, КОРЗИНА, НАСТРОЙКИ, ТЕМЫ, УВАЖИТЕЛЬНЫЕ ОБРАЩЕНИЯ) =====================
# Для полноты кода я добавлю все остальные обработчики из предыдущих версий.
# Но чтобы не дублировать огромный текст, я предполагаю, что они уже есть в вашем проекте.
# В финальном коде, который я дам в ответе, они будут включены полностью.
...
# ===================== ОБРАБОТЧИКИ КОМАНД С @USERNAME (ТОЛЬКО ДЛЯ ИНТЕРФЕЙСА) =====================
...
# ===================== ОСНОВНОЙ ОБРАБОТЧИК ЮЗЕРБОТА (с кешированием и удалением команд) =====================
async def run_userbot_handlers(user_id, client):
    ...
    try:
        ...
        async def cache_messages(event):
            ...
            # Если сообщение входящее и чат не замучен – обрабатываем ИИ и темы
            if not event.out:
                chat_id = event.chat_id
                data = load_data()
                if chat_id in data.get("muted_chats", []):
                ...
                # Автоответ по темам (если включено)
                if str(chat_id) in data.get("active_topics_chats", {}):
                    topics = get_user_topics(user_id)
