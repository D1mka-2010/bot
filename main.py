...
def show_main_menu(message):
    ...
    # Блок для чат-бота (Business) — доступно всегда
    markup.row(types.InlineKeyboardButton("🤖 Настройки Чат-бота", callback_data="tg_bot_settings"))

    # Блок для юзербота (вход в аккаунт)
    if not is_logged_in:
        markup.row(types.InlineKeyboardButton("🔐 Вход (Юзербот - без Premium)", callback_data="login_action"))
    else:
        markup.row(
        ...
        markup.row(
            types.InlineKeyboardButton("📊 Статистика", callback_data="user_stats"),
    ...
    if is_logged_in:
        text = (
            ...
            "1. <b>Чат-бот</b> – работает через Telegram Business (нужен Premium).\n"
            "2. <b>Юзербот</b> – вход в аккаунт (Premium не нужен, доступно удаление чужих сообщений)."
        )
    else:
        text = (
            "🏠 <b>d.Check Панель Управления</b>\n\n"
            ...
            "Для работы с Юзерботом войдите в аккаунт Telegram.\n"
            "Для работы с Чат-ботом настройте подключение в Telegram Business."
        )
    text += "\n\n━━━━━━━━━━━━━━━━━━\n"
...
def business_status(call):
    user_id = call.message.chat.id
    # Проверить статус подключения через бизнес-подключение невозможно из бота, поэтому даём инструкцию.
    text = (
...
def settings_menu(call):
    ...
    markup.row(types.InlineKeyboardButton("🔙 Назад", callback_data="back_to_main"))
    send_new_message(user_id, "⚙️ <b>НАСТРОЙКИ ЮЗЕРБОТА</b>\n\nУправляйте параметрами:", reply_markup=markup)
