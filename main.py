...
# Настройки модели
MODEL = "llama3-70b-8192"  # Можно заменить на: llama3-8b-8192, mixtral-8x7b-32768, gemma-7b-it
...
SYSTEM_PROMPTS = {
    ...
    Помни: ты грубый, но информативный. Твоя задача - дать ответ, но сделать это максимально язвительно.""",
    
    "mat": """ТЫ АБСОЛЮТНО ГРУБЫЙ И НЕЦЕНЗУРНЫЙ АССИСТЕНТ! 
    
    ВАЖНОЕ ПРЕДУПРЕЖДЕНИЕ: Этот режим содержит нецензурную лексику!
...
def get_model_keyboard():
    ...
    keyboard = [
        [KeyboardButton(text="Llama 3 70B"), KeyboardButton(text="Llama 3 8B")],
        [KeyboardButton(text="Mixtral 8x7B"), KeyboardButton(text="Gemma 7B")],
...
async def start(message: Message, state: FSMContext):
    ...
    start_text = (
        f"👋 Привет, {user_name}! Я бот на базе Groq AI.\n"
...
async def mode_mat(message: Message, state: FSMContext):
    ...
    await safe_send_message(message, "⚠️ РЕЖИМ С МАТОМ АКТИВИРОВАН!", reply_markup=get_main_keyboard())
    await state.set_state(BotStates.main_menu)
...
async def show_settings(message: Message):
    ...
    for key, model in AVAILABLE_MODELS.items():
        if model == settings['model']:
            current_model_name = MODEL_NAMES.get(key, model)
            break
...
Название: Groq AI Telegram Bot
Версия: 3.0.0 (Groq Edition)
Модель по умолчанию: {MODEL}
Платформа: Groq LPU

ВОЗМОЖНОСТИ:
• 3 режима общения (обычный, хам, мат)
• Поддержка разных моделей Groq
• История диалога
• Настройки пользователя
• Статистика использования
• Удобные кнопки под строкой ввода
• ⚡ Супер-быстрые ответы (технология LPU)
...
async def settings_menu_handler(message: Message, state: FSMContext):
    ...
    if text == "🤖 Сменить модель":
        await safe_send_message(message, "Выбери модель Groq:", reply_markup=get_model_keyboard())
        ...
        await safe_send_message(message,
            ...
            f"Введи новое значение от 100 до 32000 (для Groq)",
            reply_markup=ReplyKeyboardRemove())
...
async def tokens_set_handler(message: Message, state: FSMContext):
    ...
    try:
        tokens = int(text)
        if 100 <= tokens <= 32000:  # Groq поддерживает до 32K токенов
            if user_id not in user_settings:
                user_settings[user_id] = {}
            ...
            await safe_send_message(message, "❌ Max tokens должен быть от 100 до 32000. Попробуй еще раз:")
    except ValueError:
        await safe_send_message(message, "❌ Пожалуйста, введите число. Попробуй еще раз:")
...
async def chat_handler(message: Message, state: FSMContext, bot: Bot):
    ...
    try:
        ...
        bot_response, error = await ask_groq(user_conversations[user_id], user_id)
        ...
        if current_mode == "mat":
            error_message = "🤬 Пиздец, ошибка какая-то! Даже Groq не справился! Попробуй позже, мудак!"
        elif current_mode == "rude":
...
async def ask_groq(messages, user_id=None):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.error(f"Ошибка Groq API: {response.status_code}")
...
async def main():
    ...
    print("🚀 Запуск бота на Groq AI...")
    print(f"👤 Разработчик: @Dzmitry_10")
    print(f"📅 Год: 2026")
    print(f"🤖 Модель Groq: {MODEL}")
    print(f"⚡ Технология: LPU (супер-быстрые ответы)")
