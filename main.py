...
# ============================================
# НАСТРОЙКА ПРОКСИ - ВЫКЛЮЧЕНО (но код оставлен для возможности)
# ============================================
USE_PROXY = False
...
# Инициализация глобального клиента Groq (асинхронно)
async def init_groq_client(api_key):
    """Асинхронная инициализация глобального клиента Groq"""
    global groq_client
    try:
        loop = asyncio.get_running_loop()
        # Создаем HTTP клиент с прокси если нужно
        if USE_PROXY:
            proxy_url = f"socks4://{PROXY_HOST}:{PROXY_PORT}"  # или socks5
            transport = httpx.AsyncHTTPTransport(proxy=proxy_url)
            http_client = httpx.AsyncClient(transport=transport)
        else:
            http_client = httpx.AsyncClient()

        # Создаем Groq клиент с кастомным HTTP клиентом
        new_client = Groq(api_key=api_key, http_client=http_client, timeout=30.0)

        # Проверяем работоспособность ключа, сделав тестовый запрос в executor
        await loop.run_in_executor(
            None,
            lambda: new_client.chat.completions.create(
...
# Функция для обновления API ключа (асинхронная)
async def update_groq_api_key(new_api_key):
    """Обновление глобального API ключа Groq (асинхронно)"""
    global GROQ_API_KEY, groq_client
    GROQ_API_KEY = new_api_key
    success = await init_groq_client(new_api_key)
    return success
...
async def post_init(application: Application):
    ...
    try:
        ...
        success = await init_groq_client(GROQ_API_KEY)
        if not success:
            logger.error("Не удалось инициализировать Groq клиент при старте")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if user_id not in last_message_text:
        ...
        # Отправляем ответ с кнопкой навигации (без Markdown, чтобы избежать ошибок разметки)
...
async def end_dialog(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in user_data:
        current_chat = user_data[user_id].get("current_chat")
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if query.data == "owner_toggle_18_filter" and user_limits.get(user_id, {}).get("is_owner"):
            ...
            # Обновляем сообщение, возвращаясь в меню глобальных настроек
            await button_handler(update, context)  # рекурсивно вызовет повторную обработку с новым query.data?
            # Но проще вернуться в меню:
            # Создадим новое событие для owner_global_settings
            new_query = query.data = "owner_global_settings"
        ...
        if query.data == "owner_toggle_notes" and user_limits.get(user_id, {}).get("is_owner"):
            ...
            # Возвращаемся в меню глобальных настроек
            new_query = query.data = "owner_global_settings"
            await button_handler(update, context)
            return
