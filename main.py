    ...
    if user_id in awaiting_input:
        ...
        if action_data.get("action") == "new_chat_name":
            ...
            try:
                ...
                if cooldown >= 0:
                    # Применяем ко всем пользователям или к конкретному
                    target_user = action_data.get("target_user")
                    if target_user:
                        if target_user in user_limits:
                            user_limits[target_user]["cooldown"] = cooldown
                            await update.message.reply_text(f"✅ Cooldown для пользователя установлен: {cooldown} сек")
                        else:
            ...
            try:
                ...
                if limit >= 0:
                    ...
                    if target_user:
                        if target_user in user_limits:
                            user_limits[target_user]["requests_per_minute"] = limit
                            await update.message.reply_text(f"✅ Лимит запросов/мин для пользователя установлен: {limit}")
                        else:
                            await update.message.reply_text("❌ Пользователь не найден")
                    else:
                        for uid in user_limits:
    ...
    try:
        ...
        if query.data == "owner_panel" and user_limits.get(user_id, {}).get("is_owner"):
            ...
            keyboard = [
                ...
                [InlineKeyboardButton("📊 Лимиты запросов", callback_data="owner_set_requests")],
                [InlineKeyboardButton("👥 Список пользователей", callback_data="owner_user_list")],
                [InlineKeyboardButton("◀️ Назад", callback_data="back_to_main")]
            ]
            await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='Markdown')
            return
        ...
        if query.data == "owner_set_cooldown" and user_limits.get(user_id, {}).get("is_owner"):
            keyboard = [
                [InlineKeyboardButton("Для всех пользователей", callback_data="owner_cooldown_all")],
                [InlineKeyboardButton("Для конкретного пользователя", callback_data="owner_cooldown_user")],
                [InlineKeyboardButton("◀️ Назад", callback_data="owner_panel")]
            ]
            await query.edit_message_text(
                "⏱️ **Установка cooldown**\n\n"
                "Выберите, для кого установить время между запросами:",
                reply_markup=InlineKeyboardMarkup(keyboard),
                parse_mode='Markdown'
            )
            return
        
        if query.data == "owner_cooldown_all" and user_limits.get(user_id, {}).get("is_owner"):
            awaiting_input[user_id] = {"action": "set_cooldown", "target_user": None}
            await query.edit_message_text(
        ...
        if query.data == "owner_cooldown_user" and user_limits.get(user_id, {}).get("is_owner"):
            # Здесь нужно будет реализовать выбор пользователя
            # Для простоты пока просто запрашиваем ID
            awaiting_input[user_id] = {"action": "set_cooldown_user_id"}
            await query.edit_message_text(
                "⏱️ **Введите ID пользователя** (можно узнать в информации):",
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton("◀️ Отмена", callback_data="owner_panel")
                ]]),
                parse_mode='Markdown'
        ...
        # Установка лимитов запросов (только для владельца)
        if query.data == "owner_set_requests" and user_limits.get(user_id, {}).get("is_owner"):
            keyboard = [
                [InlineKeyboardButton("Запросов в минуту", callback_data="owner_set_rpm")],
                [InlineKeyboardButton("Запросов в час", callback_data="owner_set_rph")],
                [InlineKeyboardButton("Запросов в день", callback_data="owner_set_rpd")],
                [InlineKeyboardButton("◀️ Назад", callback_data="owner_panel")]
            ]
            await query.edit_message_text(
                "📊 **Установка лимитов запросов**\n\n"
                "Выберите тип лимита для настройки:",
                reply_markup=InlineKeyboardMarkup(keyboard),
                parse_mode='Markdown'
            )
            return
        ...
        if query.data == "owner_set_rpm" and user_limits.get(user_id, {}).get("is_owner"):
            awaiting_input[user_id] = {"action": "set_requests_per_minute", "target_user": None}
            await query.edit_message_text(
