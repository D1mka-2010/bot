...
def call_deepseek_ai(user_message, user_id, chat_id, history=None, sender_username=None):
    ...
    system_prompt = (
        ...
        f"Обязательно используй Markdown форматирование Telegram (звёздочки **жирный**, обратные кавычки `код`)."
    )
...
def show_main_menu(message):
    ...
    # Основные кнопки управления доступны всегда
    markup.row(types.InlineKeyboardButton("🤖 Настройки Чат-бота", callback_data="tg_bot_settings"))
    markup.row(
        types.InlineKeyboardButton("⚙️ Настройки Юзербота", callback_data="settings_menu"),
        types.InlineKeyboardButton("🤖 Управление ИИ", callback_data="ai_menu")
    )
    markup.row(
        types.InlineKeyboardButton("🔇 Управление чатами", callback_data="chats_menu"),
        types.InlineKeyboardButton("🗑️ Корзина", callback_data="view_trash")
    )
    markup.row(
        types.InlineKeyboardButton("📋 Темы", callback_data="topics_menu"),
        types.InlineKeyboardButton("🙏 Уваж. обращения", callback_data="respect_menu")
    )
    ...
    status_text = "✅ активен" if is_logged_in else "❌ не активен (войдите в Настройках Юзербота)"

    text = (
        "🏠 <b>d.Check Панель Управления</b>\n\n"
        f"👥 Пользователей: {total_users}\n"
        f"👤 Аккаунт Юзербота: <b>{status_text}</b>\n\n"
        "Выберите раздел для управления:\n"
        "1. <b>Чат-бот</b> – работает через Telegram Business.\n"
        "2. <b>Юзербот</b> – работает через ваш аккаунт."
    )
...
def settings_menu(call):
    ...
    if not is_logged_in:
        ...
        markup.row(types.InlineKeyboardButton("🚪 Выйти из аккаунта", callback_data="logout"))

    markup.row(types.InlineKeyboardButton("🔙 Назад", callback_data="back_to_main"))
    send_new_message(user_id, "⚙️ <b>НАСТРОЙКИ ЮЗЕРБОТА</b>", reply_markup=markup)
...
# ===================== НАСТРОЙКИ ЧАТ-БОТА (BUSINESS) =====================
@bot.callback_query_handler(func=lambda call: call.data == "tg_bot_settings")
def tg_bot_settings(call):
    user_id = call.message.chat.id
    text = (
        "🤖 <b>НАСТРОЙКИ ЧАТ-БОТА (Telegram Business)</b>\n\n"
        "Чат-бот работает через Telegram Business API.\n\n"
        "📌 <b>Как подключить:</b>\n"
        "1. Откройте Telegram на телефоне (нужен Premium).\n"
        "2. Настройки → Telegram Business → Чат-боты.\n"
        "3. Введите username бота: <b>@dCheck_bot</b>.\n"
        "4. Выберите, в каких чатах бот должен отвечать.\n"
        "5. Включите «Разрешить боту отвечать».\n\n"
        "⚠️ <b>Ограничения:</b>\n"
        "• Чат-бот НЕ может удалять сообщения в личных чатах.\n"
        "• Для удаления чужих сообщений используйте Юзербот.\n\n"
        "📌 <b>Как отключить:</b>\n"
        "В настройках Telegram Business удалите бота из списка."
    )
    markup = types.InlineKeyboardMarkup()
    markup.row(types.InlineKeyboardButton("🔄 Статус бизнес-подключения", callback_data="business_status"))
    markup.row(types.InlineKeyboardButton("🔙 Назад", callback_data="back_to_main"))
    send_new_message(user_id, text, reply_markup=markup)

@bot.business_connection_handler(func=lambda connection: True)
def handle_business_connection(connection):
    """Уведомление при подключении/отключении бота к бизнес-аккаунту."""
    if connection.is_enabled:
        bot.send_message(
            connection.user_chat_id,
            "✅ <b>Бот успешно подключен к Telegram Business!</b>\n\n"
            "Теперь он может работать в качестве чат-бота без необходимости прямого входа в аккаунт (Юзербот)."
        )
    else:
        bot.send_message(
            connection.user_chat_id,
            "❌ <b>Бот отключен от Telegram Business.</b>"
        )

@bot.callback_query_handler(func=lambda call: call.data == "business_status")
def business_status(call):
    user_id = call.message.chat.id
    text = (
        "🔄 <b>Статус бизнес-подключения</b>\n\n"
        "Чтобы проверить, подключен ли бот к вашему бизнес-аккаунту:\n"
        "1. Откройте Telegram → Настройки → Telegram Business.\n"
        "2. Если в разделе «Чат-боты» есть ваш бот — он подключён.\n\n"
        "Если бот не отвечает в бизнес-чатах, убедитесь, что:\n"
        "• У вас есть Telegram Premium.\n"
        "• Бот добавлен в список разрешённых.\n"
        "• Включена опция «Разрешить боту отвечать»."
    )
    markup = types.InlineKeyboardMarkup()
    markup.row(types.InlineKeyboardButton("🔙 Назад", callback_data="tg_bot_settings"))
    send_new_message(user_id, text, reply_markup=markup)
...
def handle_business_messages(message):
    ...
    owner_id = ADMIN_LIST[0] if ADMIN_LIST else None

    if owner_id and user_id != owner_id:
        return

    if command in ['/mute', '/unmute', '/ai', '/ai_off', '/check']:
        try:
        ...
        if command == '/mute':
            if chat_id not in data["muted_chats"]:
                ...
                save_data(data)
                bot.send_message(chat_id, "🔇 Чат замучен.", business_connection_id=business_connection_id)
            else:
                bot.send_message(chat_id, "ℹ️ Чат уже замучен.", business_connection_id=business_connection_id)
        elif command == '/unmute':
            if chat_id in data["muted_chats"]:
                data["muted_chats"].remove(chat_id)
                save_data(data)
                bot.send_message(chat_id, "🔊 Чат размучен.", business_connection_id=business_connection_id)
            else:
                bot.send_message(chat_id, "ℹ️ Чат не был замучен.", business_connection_id=business_connection_id)
        elif command == '/ai':
            if chat_id not in data["ai_chats"]:
                data["ai_chats"].append(chat_id)
                save_data(data)
                bot.send_message(chat_id, "🤖 ИИ включен.", business_connection_id=business_connection_id)
            else:
                bot.send_message(chat_id, "ℹ️ ИИ уже включен.", business_connection_id=business_connection_id)
        elif command == '/ai_off':
            if chat_id in data["ai_chats"]:
                data["ai_chats"].remove(chat_id)
                save_data(data)
                bot.send_message(chat_id, "🛑 ИИ выключен.", business_connection_id=business_connection_id)
            else:
                bot.send_message(chat_id, "ℹ️ ИИ не был включен.", business_connection_id=business_connection_id)
        elif command == '/check':
            if str(chat_id) in data.get("active_topics_chats", {}):
                del data["active_topics_chats"][str(chat_id)]
                save_data(data)
                bot.send_message(chat_id, "⏹ Отслеживание тем выключено.", business_connection_id=business_connection_id)
            else:
                if "active_topics_chats" not in data:
                    data["active_topics_chats"] = {}
                data["active_topics_chats"][str(chat_id)] = True
                save_data(data)
                bot.send_message(chat_id, "▶️ Отслеживание тем включено.", business_connection_id=business_connection_id)
        return
    ...
    if str(chat_id) in data.get("active_topics_chats", {}):
        topics = get_user_topics(owner_id) if owner_id else {}
        if topics and text:
            for topic_name, topic_data in topics.items():
                for example in topic_data["examples"]:
                    if example.lower() in text.lower():
                        bot.send_message(chat_id, topic_data["response"], business_connection_id=business_connection_id)
                        return

    if chat_id in data.get("ai_chats", []):
        if text and owner_id:
            sender_username = message.from_user.username if message.from_user else None
            typing_speed = get_typing_speed(owner_id)
            history = []
            reply = call_deepseek_ai(text, owner_id, chat_id, history, sender_username)
            if reply:
                bot.send_chat_action(chat_id, 'typing', business_connection_id=business_connection_id)
                time.sleep(random.uniform(1.0, 3.0))
                bot.send_message(chat_id, reply, business_connection_id=business_connection_id, parse_mode='Markdown')

# ===================== ОБЫЧНЫЕ КОМАНДЫ В ЛИЧНЫХ ЧАТАХ =====================
@bot.message_handler(commands=["mute", "unmute", "ai", "ai_off"])
def handle_user_commands(message):
    user_id = message.chat.id
    if not is_authorized(user_id):
        bot.send_message(user_id, "⚠️ Сначала войдите в аккаунт через Настройки Юзербота → Войти в аккаунт", parse_mode='HTML')
        return
    if is_command_blocked(message.chat.id):
        return
    if message.chat.type != "private":
        bot.send_message(user_id, "⚠️ Команды с @username работают только в ЛС с ботом!\n\nДля работы в группах используйте юзербота.", parse_mode='HTML')
        return
    ...
    # Здесь можно добавить логику обработки через юзербота или просто подсказку
    bot.send_message(user_id, "ℹ️ Команды с @username выполняются юзерботом в любом чате. Просто напишите команду там, где нужно.")
...
def auth_continue_step1(call):
    ...
    markup = types.InlineKeyboardMarkup()
    markup.row(types.InlineKeyboardButton("🔙 Инструкция", callback_data="auth_back_to_instruction"))
    markup.row(types.InlineKeyboardButton("❌ Отменить вход", callback_data="cancel_auth"))
    delete_old_bot_message(user_id)
    msg = send_new_message(user_id, text, reply_markup=markup)
    bot.register_next_step_handler(msg, process_api_id)
...
def process_api_id(message):
    ...
    try:
        ...
        text = (
            "🔐 <b>АВТОРИЗАЦИЯ (Способ 1)</b>\n\n"
            "✅ API_ID принят!\n"
            "➡️ Введите ваш API_HASH (строка из букв и цифр):"
        )
        markup = types.InlineKeyboardMarkup()
        markup.row(types.InlineKeyboardButton("🔙 Шаг 1", callback_data="auth_back_to_api_id"))
        markup.row(types.InlineKeyboardButton("❌ Отменить вход", callback_data="cancel_auth"))
        delete_old_bot_message(user_id)
        msg = send_new_message(user_id, text, reply_markup=markup)
        bot.register_next_step_handler(msg, process_api_hash)
    except ValueError:
        send_new_message(user_id, "❌ API_ID должен состоять только из цифр. Попробуйте снова.")
...
def auth_back_to_api_id(call):
    user_id = call.message.chat.id
    auth_data[user_id]["step"] = "api_id"
    text = "Введите API_ID (цифры):"
    markup = types.InlineKeyboardMarkup()
    markup.row(types.InlineKeyboardButton("🔙 Назад", callback_data="auth_back_to_instruction"))
    markup.row(types.InlineKeyboardButton("❌ Отменить", callback_data="cancel_auth"))
    msg = send_new_message(user_id, text, reply_markup=markup)
    bot.register_next_step_handler(msg, process_api_id)
...
def process_api_hash(message):
    ...
    markup = types.InlineKeyboardMarkup()
    markup.row(types.InlineKeyboardButton("🔙 Шаг 2", callback_data="auth_back_to_api_hash"))
    markup.row(types.InlineKeyboardButton("❌ Отменить вход", callback_data="cancel_auth"))
    delete_old_bot_message(user_id)
    msg = send_new_message(user_id, text, reply_markup=markup)
    bot.register_next_step_handler(msg, process_phone_method_1)
...
def process_phone_method_1(message):
    ...
    if not phone.startswith("+"):
        delete_old_bot_message(user_id)
        msg = send_new_message(user_id, "❌ Номер должен начинаться с +.\nПопробуйте снова:")
        bot.register_next_step_handler(msg, process_phone_method_1)
        return
    ...
    delete_old_bot_message(user_id)
    markup = types.InlineKeyboardMarkup()
    markup.row(types.InlineKeyboardButton("✅ Продолжить", callback_data="confirm_phone_step1"))
    markup.row(types.InlineKeyboardButton("✏️ Изменить", callback_data="edit_phone_step1"))
    send_new_message(user_id, f"📱 Введённый номер: <code>{phone}</code>\nВсё верно?", reply_markup=markup)

@bot.callback_query_handler(func=lambda call: call.data == "edit_phone_step1")
def edit_phone_step1(call):
    user_id = call.message.chat.id
    delete_old_bot_message(user_id)
    msg = send_new_message(user_id, "📱 Введите номер заново (формат: +375291234567):")
    bot.register_next_step_handler(msg, process_phone_method_1)
...
def confirm_phone_step1(call):
    ...
    if user_id not in pending_phone:
        bot.answer_callback_query(call.id, "⚠️ Номер не найден. Начните заново.")
        return
    ...
    try:
        ...
        send_new_message(user_id, f"❌ Ошибка отправки кода: {e}")
...
def handle_code_input(call):
    ...
    if action == "done":
        ...
        if len(code) < 4:
            bot.answer_callback_query(call.id, "❌ Код слишком короткий!", show_alert=True)
            return
        method = get_auth_method(user_id)
        if method == 1:
...
def process_code_submit_method_1(message, code, user_id):
    if user_id not in auth_data:
        send_new_message(user_id, "⚠️ Сессия не найдена. Начните заново.")
        if user_id in auth_in_progress:
            del auth_in_progress[user_id]
        return
    try:
        ...
        save_user_session(user_id, new_session_string, auth_data[user_id]["api_id"], auth_data[user_id]["api_hash"])
        user_data = get_user(user_id) or {}
        user_data.update({"logged_in": True, "phone": auth_data[user_id]["phone"]})

        # Проверка Premium-статуса и username
        try:
            ...
            if me:
                ...
                if getattr(me, 'premium', False):
                    ...
                    bot.send_message(message.chat.id, "⭐️ <b>У вас активен Telegram Premium!</b>")
                else:
...
def logout_handler(call):
    ...
    settings_menu(call)  # возвращаем в настройки

# ===================== НАСТРОЙКИ ЮЗЕРБОТА (подменю) =====================
...
def auth_settings_menu(call):
    ...
    markup.row(types.InlineKeyboardButton("🔙 Назад", callback_data="settings_menu"))
    send_new_message(user_id, text, reply_markup=markup)
...
def switch_to_method_1(call):
    user_id = call.message.chat.id
    set_auth_method(user_id, 1)
    bot.answer_callback_query(call.id, "✅ Переключено на способ 1")
...
def switch_to_method_7(call):
    user_id = call.message.chat.id
    set_auth_method(user_id, 7)
    bot.answer_callback_query(call.id, "✅ Переключено на способ 7")
    auth_settings_menu(call)
...
def toggle_notifications_menu(call):
    ...
    text = "🔔 <b>УВЕДОМЛЕНИЯ ОБ УДАЛЕНИИ</b>\n\n"
    text += "🟢 Включены" if show else "🔴 Отключены"
    markup = types.InlineKeyboardMarkup()
    if show:
        markup.row(types.InlineKeyboardButton("❌ Отключить", callback_data="notifications_off"))
    else:
        markup.row(types.InlineKeyboardButton("✅ Включить", callback_data="notifications_on"))
    markup.row(types.InlineKeyboardButton("🔙 Назад", callback_data="settings_menu"))
    send_new_message(user_id, text, reply_markup=markup)
...
def typing_speed_menu(call):
    ...
    if not is_authorized(user_id):
        bot.answer_callback_query(call.id, "⚠️ Сначала войдите в аккаунт!", show_alert=True)
        return
    current = get_typing_speed(user_id)
    text = f"⌨️ <b>СКОРОСТЬ ПЕЧАТИ</b>\n\nТекущая: {current} символов/сек"
    markup = types.InlineKeyboardMarkup(row_width=3)
    for s in [1,2,3,4,5,7,10,15,20]:
        markup.add(types.InlineKeyboardButton(str(s), callback_data=f"speed_{s}"))
    markup.row(types.InlineKeyboardButton("🔙 Назад", callback_data="settings_menu"))
    send_new_message(user_id, text, reply_markup=markup)

@bot.callback_query_handler(func=lambda call: call.data.startswith("speed_"))
def set_typing_speed(call):
    speed = int(call.data.split("_")[1])
    user_id = call.message.chat.id
    set_typing_speed(user_id, speed)
    bot.answer_callback_query(call.id, f"✅ Скорость {speed} символов/сек")
...
def toggle_commands_menu(call):
    ...
    text = "⚙️ <b>УПРАВЛЕНИЕ КОМАНДАМИ</b>\n\n"
    text += "🔴 Команды отключены" if blocked else "🟢 Команды включены"
    markup = types.InlineKeyboardMarkup()
    if blocked:
        markup.row(types.InlineKeyboardButton("✅ Включить", callback_data="commands_on"))
    else:
        markup.row(types.InlineKeyboardButton("❌ Отключить", callback_data="commands_off"))
    markup.row(types.InlineKeyboardButton("🔙 Назад", callback_data="settings_menu"))
...
def commands_on(call):
    ...
    if str(chat_id) in data.get("command_blacklist", []):
        ...
        save_data(data)
    bot.answer_callback_query(call.id, "✅ Команды включены")
...
def commands_off(call):
    ...
    if str(chat_id) not in data["command_blacklist"]:
        ...
        save_data(data)
    bot.answer_callback_query(call.id, "❌ Команды отключены")
    toggle_commands_menu(call)
...
def ai_menu(call):
    ...
    text = f"🤖 <b>УПРАВЛЕНИЕ ИИ</b>\n\nАктивных чатов с ИИ: {len(ai_chats)}"
    markup = types.InlineKeyboardMarkup()
...
def view_ai_chats(call):
    ...
    markup = types.InlineKeyboardMarkup()
    markup.row(types.InlineKeyboardButton("🔙 Назад", callback_data="ai_menu"))
    send_new_message(user_id, text, reply_markup=markup)
...
def setup_ai_start(call):
    ...
    if not is_authorized(user_id):
        bot.answer_callback_query(call.id, "⚠️ Сначала войдите в аккаунт!", show_alert=True)
        return
    msg = send_new_message(user_id, "🤖 <b>НАСТРОЙКА ИИ</b>\n\nШаг 1: Как называть твоего ИИ-двойника?")
...
def chats_menu(call):
    ...
    text = f"🔇 <b>УПРАВЛЕНИЕ ЧАТАМИ</b>\n\nЗамученных чатов: {len(muted_chats)}"
    markup = types.InlineKeyboardMarkup()
...
def view_muted_chats(call):
    ...
    text = "🔇 <b>Замученные чаты:</b>\n\n" + ("\n".join([f"• ID: <code>{cid}</code>" for cid in muted]) if muted else "Нет замученных чатов.")
    markup = types.InlineKeyboardMarkup()
    markup.row(types.InlineKeyboardButton("🔙 Назад", callback_data="chats_menu"))
    send_new_message(user_id, text, reply_markup=markup)
...
def clean_chats_menu(call):
    ...
    if clean_chats:
        text += "Чаты с автоочисткой:\n" + "\n".join([f"• ID: <code>{cid}</code>" for cid in clean_chats])
    else:
        text += "Нет чатов с автоочисткой."
    text += "\n\nИспользуйте команды /clean_on и /clean_off в чате (бот должен быть админом)."
    markup = types.InlineKeyboardMarkup()
...
def user_commands_handler(call):
    ...
    text = (
        ...
        "Используйте команды с @username в ЛС с ботом:\n\n"
        "🔹 <code>/mute @username</code> - замутить чат\n"
        "🔹 <code>/unmute @username</code> - размутить\n"
        "🔹 <code>/ai @username</code> - включить ИИ\n"
        "🔹 <code>/ai_off @username</code> - выключить ИИ\n"
        "🔹 <code>/check</code> - в любом чате включить/выключить темы\n\n"
        "⚠️ Команды удаляются автоматически, собеседник их не видит."
    )
    markup = types.InlineKeyboardMarkup()
    markup.row(types.InlineKeyboardButton("🔙 Назад", callback_data="chats_menu"))
    send_new_message(user_id, text, reply_markup=markup)
...
def view_trash_user_msgs(call):
    ...
    markup.row(
        ...
        types.InlineKeyboardButton("🔙 К списку", callback_data="view_trash")
    )
    delete_old_bot_message(user_id)
    send_new_message(user_id, text, reply_markup=markup)
...
def trash_clear_user(call):
    ...
    if uid in data.get("deleted_messages", {}) and sender_id in data["deleted_messages"][uid]:
        ...
        save_data(data)
    bot.answer_callback_query(call.id, "✅ Сообщения пользователя удалены!")
    view_trash_users(call)
...
def user_stats_menu(call):
    ...
    text = (
        ...
        f"🔇 Замученных: {len(data.get('muted_chats', []))}\n"
        f"🗑️ Сообщений в корзине: {sum(len(v) for v in data.get('deleted_messages', {}).values())}\n\n"
        f"👤 Ваша статистика:\n"
        f"• Сообщений: {user_messages}\n"
        f"• Скорость печати: {typing_speed} символов/сек\n"
        f"• Первое использование: {user_first_use}\n"
        f"• Последнее: {user_last_use}"
    )
    markup = types.InlineKeyboardMarkup()
...
def topics_menu(call):
    ...
    markup.row(types.InlineKeyboardButton("🔙 Назад", callback_data="back_to_main"))
    send_new_message(user_id, "📋 <b>УПРАВЛЕНИЕ ТЕМАМИ</b>\n\nСоздавайте темы для автоматических ответов.", reply_markup=markup)
...
def add_topic_start(call):
    ...
    msg = send_new_message(user_id, "📝 <b>Введите название темы</b>")
...
def process_topic_name(message):
    ...
    if not topic_name:
        send_new_message(user_id, "❌ Название не может быть пустым")
        add_topic_start(message)
        return
    topic_creation_data[user_id]["name"] = topic_name
    msg = send_new_message(user_id, "📝 <b>Введите пример сообщения</b> (или /done)")
...
def process_topic_example(message):
    ...
    if message.text == "/done":
        if len(topic_creation_data[user_id]["examples"]) == 0:
            send_new_message(user_id, "❌ Добавьте хотя бы один пример")
            msg = send_new_message(user_id, "📝 Введите пример (или /done)")
            bot.register_next_step_handler(msg, process_topic_example)
            return
        msg = send_new_message(user_id, "💬 <b>Введите ответ бота</b>")
    ...
    if message.text:
        ...
        send_new_message(user_id, "❌ Отправьте текст")
        msg = send_new_message(user_id, "📝 Введите пример (или /done)")
        bot.register_next_step_handler(msg, process_topic_example)
...
def process_topic_response(message):
    ...
    if not response:
        send_new_message(user_id, "❌ Ответ не может быть пустым")
        msg = send_new_message(user_id, "💬 Введите ответ")
        bot.register_next_step_handler(msg, process_topic_response)
        return
    ...
    send_new_message(user_id, f"✅ <b>Тема '{topic_name}' создана!</b>\nПримеров: {len(examples)}")
    topics_menu(message)
...
def list_topics(call):
    ...
    if not topics:
        send_new_message(user_id, "📭 Нет тем")
        topics_menu(call)
        return
    text = "📋 <b>Ваши темы:</b>\n\n"
    for name, data in topics.items():
        text += f"🔹 <b>{name}</b> (примеров: {len(data['examples'])})\n"
    markup = types.InlineKeyboardMarkup()
...
def delete_topic_confirm(call):
    ...
    if delete_user_topic(user_id, topic_name):
        bot.answer_callback_query(call.id, f"✅ Тема '{topic_name}' удалена")
    else:
...
def respect_menu(call):
    ...
    if not is_authorized(user_id):
        ...
        return
    markup = types.InlineKeyboardMarkup()
    markup.row(types.InlineKeyboardButton("➕ Добавить", callback_data="add_respect"))
...
def process_add_respect(message):
    ...
    if add_respected_user(user_id, username):
        send_new_message(user_id, f"✅ {username} добавлен")
    else:
...
def list_respect(call):
    ...
    text = "📋 <b>Список:</b>\n\n" + ("\n".join([f"• {u}" for u in users]) if users else "Список пуст")
    markup = types.InlineKeyboardMarkup()
...
def remove_respect_menu(call):
    ...
    users = get_respected_users(user_id)
    if not users:
        send_new_message(user_id, "📭 Нет пользователей")
        respect_menu(call)
        return
    markup = types.InlineKeyboardMarkup()
    for u in users:
    ...
    markup.row(types.InlineKeyboardButton("🔙 Назад", callback_data="respect_menu"))
    send_new_message(user_id, "Выберите пользователя для удаления:", reply_markup=markup)
...
def remove_respect_confirm(call):
    ...
    if remove_respected_user(user_id, username):
        bot.answer_callback_query(call.id, f"✅ {username} удалён")
    else:
        bot.answer_callback_query(call.id, "❌ Ошибка")
...
def admin_stats(call):
    ...
    stats_text = (
        ...
        f"🔢 Активных юзерботов: {len(active_clients)}\n"
        f"🗑️ Сообщений в корзине: {sum(len(v) for v in data.get('deleted_messages', {}).values())}\n"
...
# ===================== УПРАВЛЕНИЕ ЮЗЕРБОТАМИ (С ПРОВЕРКОЙ АВТОРИЗАЦИИ) =====================
def start_userbot(user_id):
    if user_id in active_clients:
        print(f"⚠️ Юзербот для {user_id} уже запущен")
        return
    method = get_auth_method(user_id)
    if method == 1:
        session_data = get_user_session_data(user_id)
        ...
        if not session_file_exists(user_id):
            print(f"❌ Нет файла сессии для {user_id}")
            return
...
async def run_userbot_method_1(user_id, session_data):
    ...
    try:
        await client.connect()
        # ВАЖНО: Проверяем, жива ли сессия
        if not await client.is_user_authorized():
            print(f"❌ [{user_id}] Сессия недействительна или отключена.")
            user_data = get_user(user_id) or {}
            if user_data:
        ...
        print(f"❌ [{user_id}] Ошибка подключения к серверам Telegram.")
        user_data = get_user(user_id) or {}
        if user_data:
            user_data["logged_in"] = False
            save_user(user_id, user_data)
...
async def run_userbot_handlers(user_id, client):
    ...
    try:
        # Обработка исходящих команд (удаление в любых чатах)
        ...
        # Кеширование сообщений (только группы)
        ...
        async def cache_messages(event):
            ...
            if event.sender:
                sender_name = getattr(event.sender, 'first_name', str(event.sender_id))
            message_cache[event.id] = {
                "text": event.message.text or "[Медиа/Стикер/Файл]",
            ...
            # Обработка входящих сообщений (ИИ, темы, мут)
            if not event.out:
                chat_id = event.chat_id
                data = load_data()

                # Мут – удаляем сообщение собеседника
                if chat_id in data.get("muted_chats", []):
                    try:
                        await event.delete()
                    except:
                ...
                if str(chat_id) in data.get("active_topics_chats", {}):
                    topics = get_user_topics(user_id)
                    if topics and event.text:
                        for topic_name, topic_data in topics.items():
                            for example in topic_data["examples"]:
                                if example.lower() in event.text.lower():
                                    await event.reply(topic_data["response"])
                                    return
        ...
        # Отслеживание удалений (используем кэш)
        @client.on(events.MessageDeleted)
        async def handle_deleted(event):
            if event.is_private:
                return
            if event.original_update and hasattr(event.original_update, 'peer_id') and event.original_update.peer_id.user_id == my_id:
                return
            for msg_id in event.deleted_ids:
                if msg_id in message_cache:
                    ...
                    if data.get("show_deleted_notifications", True):
                        try:
                            notification_text = f"🗑 <b>Удалено сообщение от {msg_data['sender_name']}</b>:\n\n📝 <i>{msg_data['text']}</i>"
...
if __name__ == "__main__":
    ...
    # Запускаем автозапуск юзерботов в отдельном потоке
    def start_auto():
        loop.run_until_complete(auto_start_userbots())
    auto_thread = threading.Thread(target=start_auto, daemon=True)
    auto_thread.start()

    # Запускаем Telebot (основной поток)
    run_telebot()
