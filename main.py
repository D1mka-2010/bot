...
# ===================== НАСТРОЙКИ =====================
# ВАШИ ТОКЕНЫ (из предыдущего кода)
BOT_TOKEN = "8817363723:AAGJQG38Pyd7caE9f4LWrpBwy-A44O5kDK0"
DEEPSEEK_API_KEY = "sk-5e32f44e18f04897a8a4b1f94b52f482"
API_ID = 28432904
API_HASH = "2150e2107b519bd98d2a2a9057510373"
ADMIN_IDS = "7666021527"  # можно указать несколько через запятую

ADMIN_LIST = [int(x.strip()) for x in ADMIN_IDS.split(",") if x.strip()]
SUPPORT_IDS = ""  # при желании добавьте ID поддержки
...
def handle_user_commands(message):
    user_id = message.chat.id
    # Отладка
    print(f"[DEBUG] Команда от {user_id}: {message.text}")
    print(f"[DEBUG] is_command_blocked? {is_command_blocked(message.chat.id)}")
    print(f"[DEBUG] is_authorized? {is_authorized(user_id)}")
    ...
    if is_command_blocked(message.chat.id):
        print("[DEBUG] Команда заблокирована для этого чата")
        return
...
def reset_state(message):
    ...
    code_input_data.pop(user_id, None)
    auth_in_progress.pop(user_id, None)
    pending_phone.pop(user_id, None)
    send_new_message(user_id, "✅ Все состояния сброшены. Попробуйте команду заново.")
...
def clear_blacklist(message):
    ...
    data["command_blacklist"] = []
    save_data(data)
    send_new_message(message.chat.id, "✅ Чёрный список команд очищен.")
