...
MODES = {
    "normal": {
        "name": "💬 Обычный",
        "emoji": "💬",
        "system_prompt": "Ты вежливый и полезный ассистент. Отвечай кратко и по делу. Ты НЕ должен отвечать на запросы 18+ контента, порнографии, насилия или других неприемлемых тем. Если пользователь просит что-то подобное, вежливо откажись и предложи другую тему.",
        "description": "Вежливые и полезные ответы"
    },
    "ham": {
        "name": "😈 Хам",
        "emoji": "😈",
        "system_prompt": "Ты грубый, саркастичный и дерзкий ассистент-хам. Отвечай с издевкой, используй сарказм, подкалывай пользователя. Будь максимально дерзким и язвительным. Но даже будучи хамом, ты НЕ должен отвечать на запросы 18+ контента, порнографии, насилия или других неприемлемых тем. Если пользователь просит что-то подобное, грубо откажи и высмей его.",
        "description": "Грубые и саркастичные ответы"
    },
    "swear": {
        "name": "🤬 Мат",
        "emoji": "🤬",
        "system_prompt": "Ты ассистент, который постоянно использует нецензурную лексику. Отвечай грубо, с матом через каждое слово. Но даже используя мат, ты НЕ должен отвечать на запросы 18+ контента, порнографии, насилия или других неприемлемых тем. Если пользователь просит что-то подобное, откажи с использованием мата.",
...
MAX_ADULT_ATTEMPTS = 3  # Максимальное количество попыток 18+ запросов до предупреждения
...
async def post_init(application: Application):
    ...
    try:
        ...
        commands = [
            ...
            BotCommand("activity", "Активность пользователей"),
        ]
        ...
        if OWNER_ID:
            await application.bot.send_message(
                ...
                text="✅ **Бот успешно запущен!**\n\nВсе системы работают в штатном режиме.\n\n"
                     "🔞 **Фильтр 18+ контента активирован**\n"
                     "📊 **Система отслеживания активности включена**",
                parse_mode='Markdown'
            )
    except Exception as e:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_text = (
        ...
        f"/info - информация\n"
        f"/chats - мои чаты\n"
        f"/activity - активность (только для админа)\n\n"
        f"🔞 **Важно:** Бот не отвечает на запросы 18+ контента"
    )
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    help_text = (
        ...
        f"/activity - активность пользователей (только админ)\n\n"
        ...
        f"🔞 **Политика контента:**\n"
        f"• Бот НЕ отвечает на запросы 18+ контента\n"
        f"• За попытки получить такой контент выдаются предупреждения\n"
        f"• После {MAX_WARNINGS} предупреждений - бан"
    )
...
async def settings_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"• Предупреждений: {user_data[user_id].get('warnings', 0)}/{MAX_WARNINGS}\n\n"
        f"🔧 **Управление:**\n"
...
async def activity_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Команда для просмотра активности пользователей (только для админа)"""
    user_id = update.effective_user.id
    
    # Проверяем, является ли пользователь владельцем
    if not user_limits.get(user_id, {}).get("is_owner", False):
    ...
    user_request_counts = {uid: len(times) for uid, times in user_requests.items()}
