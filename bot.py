...
# ===================== НОВАЯ АВТОРИЗАЦИЯ СПОСОБ 1 (ПОШАГОВО) =====================
...
# ===================== ОБРАБОТЧИКИ КОМАНД С @USERNAME (ОСТАВЛЯЕМ ТОЛЬКО ДЛЯ ИНТЕРФЕЙСА, ЮЗЕРБОТ БУДЕТ ИХ ПЕРЕХВАТЫВАТЬ) =====================
@bot.message_handler(commands=["mute", "unmute", "ai", "ai_off"])
def handle_user_commands(message):
    # Этот обработчик остаётся только для информации, но реальная работа через юзербота
    user_id = message.chat.id
    if not is_authorized(user_id):
...
# ===================== УПРАВЛЕНИЕ ЮЗЕРБОТАМИ (ИСПРАВЛЕННОЕ) =====================
...
# ===================== ИСПРАВЛЕННЫЙ run_userbot_handlers (с кешированием всех сообщений + обработка исходящих команд) =====================
async def run_userbot_handlers(user_id, client):
    ...
    # Получаем информацию о себе (свой ID)
    me = await client.get_me()
    my_id = me.id

    try:
        # ========== НОВОЕ: Обработка исходящих команд в любых чатах ==========
        @client.on(events.NewMessage(outgoing=True))
        async def handle_outgoing_commands(event):
            # Проверяем, что это команда
            if not event.text or not event.text.startswith('/'):
                return
            # Игнорируем команды в личке с ботом (чтобы не мешать интерфейсу)
            if event.chat_id == user_id or event.chat_id == BOT_ID:
                return
            # Если команды отключены в этом чате – выходим
            ...
            # 1. Сразу удаляем команду из чата (собеседник не увидит)
            await event.delete()
            ...
            if command == '/mute':
                ...
                # Переключаем режим постоянного отслеживания тем в этом чате
        ...
        async def message_handler(event):
            # Игнорируем сообщения от бота и в чате с самим собой
            if event.sender_id == BOT_ID or (BOT_ID is not None and event.chat_id == BOT_ID):
                return
            if event.chat_id == user_id:
                return
            # Игнорируем свои исходящие сообщения (кроме команд, уже обработано выше)
            if event.out:
                return
            # Кешируем все сообщения (и исходящие, и входящие, и в группах)
            if event.message:
                try:
                ...
                # Обрабатываем только приватные неисходящие сообщения для мута и ИИ
