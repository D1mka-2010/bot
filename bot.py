...
# Для постоянного хранения на Render используем /data (если смонтирован том)
...
def start_userbot(user_id):
    ...
    def run_client():
        ...
        # ======== ИСПРАВЛЕНИЕ 1: Игнорируем удаления в ЛС ========
        @client.on(events.MessageDeleted)
        async def handle_deleted_messages(event):
            # Игнорируем удаления в личных сообщениях (чат с ботом)
            if event.is_private:
                return
