...
# ===================== НАСТРОЙКИ =====================
BOT_TOKEN = "8817363723:AAH248lhECJRhIE80CXBVSfe-JwXQrm-37A"
API_ID = 28432904
API_HASH = "2150e2107b519bd98d2a2a9057510373"
ADMIN_IDS = "7666021527"
ADMIN_LIST = [7666021527]
SUPPORT_IDS = ""
SUPPORT_LIST = []
DEEPSEEK_API_KEY = "sk-5e32f44e18f04897a8a4b1f94b52f482"
DEEPSEEK_URL = "https://api.deepseek.com/v1/chat/completions"

# Для постоянного хранения (если смонтирован том /data)
DATA_DIR = os.environ.get("DATA_DIR", ".")
CONFIG_FILE = os.path.join(DATA_DIR, "dcheck_data.json")
...
# ===================== АВТОРИЗАЦИЯ (ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ) =====================
code_input_data = {}
auth_data = {}
auth_in_progress = {}
pending_phone = {}
...
def auth_settings_menu(call):
    ...
    text = f"🔐 <b>НАСТРОЙКИ АВТОРИЗАЦИИ</b>\n\nТекущий метод: <b>{'Ввод API (способ 1)' if current_method == 1 else 'Файлы сессий (способ 7)'}</b>"
    markup = types.InlineKeyboardMarkup()
    if current_method == 1:
...
def typing_speed_menu(call):
    ...
    text = f"⌨️ <b>СКОРОСТЬ ПЕЧАТИ</b>\n\nТекущая: {current_speed} символов/сек"
...
def toggle_commands_menu(call):
    ...
    text += "🔴 Команды отключены в этом чате" if blocked else "🟢 Команды включены в этом чате"
    markup = types.InlineKeyboardMarkup()
    if blocked:
...
def logout_handler(call):
    ...
    save_data(data)
    stop_userbot(user_id)
    send_new_message(user_id, "🚪 <b>Вы вышли из аккаунта.</b>")
    show_main_menu(call.message)
...
def admin_panel(call):
    ...
    markup = types.InlineKeyboardMarkup()
    markup.row(types.InlineKeyboardButton("➕ Добавить поддержку", callback_data="admin_add_support"))
    markup.row(types.InlineKeyboardButton("➖ Удалить поддержку", callback_data="admin_remove_support"))
...
def admin_add_support(call):
    ...
    if not is_admin(user_id):
        bot.answer_callback_query(call.id, "⛔ Доступ запрещен!")
        return
    msg = send_new_message(user_id, "📝 Введите ID пользователя:")
...
def process_add_support(message):
    ...
    try:
        ...
        if "support_list" not in data:
            data["support_list"] = []
        if uid not in data["support_list"]:
            data["support_list"].append(uid)
            save_data(data)
            send_new_message(user_id, f"✅ Пользователь {uid} добавлен в поддержку")
        else:
            send_new_message(user_id, f"ℹ️ {uid} уже в поддержке")
    except:
        send_new_message(user_id, "❌ Неверный ID")
    admin_panel(message)
...
def admin_remove_support(call):
    ...
    support_list = data.get("support_list", [])
    if not support_list:
        send_new_message(user_id, "ℹ️ Список пуст")
        admin_panel(call)
        return
    markup = types.InlineKeyboardMarkup()
    for uid in support_list:
...
def admin_remove_support_user(call):
    ...
    data = load_data()
    if uid in data.get("support_list", []):
        data["support_list"].remove(uid)
        save_data(data)
        bot.answer_callback_query(call.id, f"✅ {uid} удалён из поддержки")
    else:
        bot.answer_callback_query(call.id, "ℹ️ Не найден")
    admin_panel(call)
...
def admin_stats(call):
    ...
    text = (
        ...
        f"🔢 Активных юзерботов: {len(user_tasks)}\n"
        f"🗑️ Сообщений в корзине: {sum(len(v) for v in data.get('deleted_messages', {}).values())}\n"
...
def show_tos(message):
    ...
    markup = types.InlineKeyboardMarkup()
    markup.row(types.InlineKeyboardButton("✅ ПРИНЯТЬ", callback_data="accept_tos"))
...
def accept_tos(call):
    ...
    data["tos_versions"][str(user_id)].append({"version": TOS_VERSION, "date": user_data["tos_date"]})
    save_data(data)
    delete_old_bot_message(user_id)
...
def decline_tos(call):
    user_id = call.message.chat.id
    delete_old_bot_message(user_id)
    send_new_message(user_id, "❌ Вы отказались. Без принятия работа невозможна.")
...
def my_tos(call):
    ...
    if not versions:
        text = "📋 <b>Мои соглашения</b>\n\nВы ещё не принимали соглашение."
    else:
        text = "📋 <b>ИСТОРИЯ</b>\n\n"
        for v in reversed(versions):
            text += f"✅ Версия {v['version']} от {v['date']}\n"
    markup = types.InlineKeyboardMarkup()
...
async def userbot_main(user_id):
    ...
    if method == 1:
        ...
        if not session_file_exists(user_id):
            return
        client = TelegramClient(get_session_file(user_id), API_ID, API_HASH)

    me = await client.get_me()
    my_id = me.id

    @client.on(events.NewMessage(outgoing=True))
    async def handle_my_commands(event):
    ...
    async def auto_reply_topics(event):
        if event.sender_id == my_id or event.is_private:
            return
        chat_id = event.chat_id
        data = load_data()
        if str(chat_id) not in data.get("active_topics_chats", {}):
            return
    ...
    async def ai_reply(event):
        ...
        chat_id = event.chat_id
        data = load_data()
        if chat_id not in data.get("ai_chats", []):
            return
        if chat_id in data.get("muted_chats", []):
            return
        text = event.text
        ...
        try:
            ...
            pass
        reply = call_deepseek_ai(text, user_id, chat_id, history, sender_username)
        if reply:
    ...
    async def cache_all_messages(event):
        if event.sender_id == my_id or event.is_private:
            return
    ...
    await client.start()
    print(f"✅ Юзербот для {user_id} запущен")
...
def start_userbot(user_id):
    ...
    def run():
        ...
        try:
            loop.run_until_complete(userbot_main(user_id))
        except Exception as e:
            print(f"❌ Ошибка юзербота {user_id}: {e}")
        finally:
            loop.close()
    thread = threading.Thread(target=run, daemon=True)
    thread.start()
    user_tasks[user_id] = thread
    print(f"✅ Юзербот для {user_id} запущен в фоне")

def stop_userbot(user_id):
    if user_id in user_tasks:
        del user_tasks[user_id]
        print(f"🛑 Юзербот для {user_id} остановлен")
...
@app.route('/')
def home():
    return jsonify({"status": "online", "bot": "d.Check", "users": get_total_users(), "active_userbots": len(user_tasks)})
...
if __name__ == "__main__":
    ...
    # Автозапуск юзерботов
    data = load_data()
    for uid, u_data in data.get("users", {}).items():
        if u_data.get("logged_in", False):
            start_userbot(int(uid))

    # Запуск telebot в потоке
