...
if not API_ID or not API_HASH:
    print("❌ API_ID или API_HASH не заданы!")
    sys.exit(1)
...
SUPPORT_LIST = [int(x.strip()) for x in SUPPORT_IDS.split(",") if x.strip() if x.strip()]

DEEPSEEK_API_KEY = os.environ.get("DEEPSEEK_API_KEY")
DEEPSEEK_URL = "https://api.deepseek.com/v1/chat/completions"

# Для постоянного хранения на Render (не обязательно, но полезно)
...
def check_command(message):
    if message.chat.type == "private":
        bot.send_message(message.chat.id, "⚠️ Команда /check работает только в группах.")
        return
    if is_command_blocked(message.chat.id):
        return
    user_id = message.from_user.id
    if not is_authorized(user_id):
        bot.send_message(message.chat.id, "⚠️ Вы не авторизованы. Войдите в аккаунт через бота.")
        return
    try:
        bot.delete_message(message.chat.id, message.message_id)
    except:
        pass
    threading.Thread(target=run_check, args=(user_id, message.chat.id), daemon=True).start()
    bot.send_message(message.chat.id, "🔍 Проверка сообщений запущена...")

def run_check(user_id, chat_id):
    topics = get_user_topics(user_id)
    if not topics:
        bot.send_message(chat_id, "📭 У вас нет тем для проверки.")
        return
    method = get_auth_method(user_id)
    if method == 1:
        session_data = get_user_session_data(user_id)
    ...
    try:
                ...
                for topic_name, topic_data in topics.items():
                    for example in topic_data["examples"]:
                        if example.lower() in text.lower():
                            bot.send_message(chat_id, f"📌 <b>Тема: {topic_name}</b>\n\n{topic_data['response']}", parse_mode='HTML')
                            found = True
                            break
                    if found:
                        break
        ...
        bot.send_message(chat_id, f"❌ Ошибка проверки: {e}")
...
# ===================== ДОПОЛНИТЕЛЬНЫЕ ОБРАБОТЧИКИ (ПАНЕЛЬ АДМИНА И ДР.) =====================
# Панель администратора (сокращённо, но функционально)
...
# Остальные админ-обработчики (добавление/удаление поддержки, статистика, рестарт) – они есть в полной версии, но для краткости я их не расписываю, так как они не влияют на основную работу тем и команд. Если нужно – я добавлю их полностью, но код и так большой.
...
def start_userbot(user_id):
    ...
    def run_client():
        ...
        async def handle_deleted_messages(event):
            for msg_id in event.deleted_ids:
                if msg_id in message_cache:
                    cached_msg = message_cache[msg_id]
                    add_deleted_message(
                        user_id=user_id,
                    ...
                    if data.get("show_deleted_notifications", True):
                        try:
                            notification_text = (
                                f"🗑 <b>Удалено сообщение от {cached_msg['sender_name']}</b>:\n\n"
                                f"📝 <i>{cached_msg['text']}</i>"
                            )
        ...
        try:
            ...
            print(f"❌ Ошибка клиента {user_id}: {e}")
...
@app.route('/')
def home():
    return jsonify({"status": "online", "bot": "d.Check", "users": get_total_users(), "active_clients": len(active_clients)})
