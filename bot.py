...
# ========== НОВОЕ: функции для работы с темами и уважительными обращениями ==========
...
# ===== ФУНКЦИЯ ОБНОВЛЕНИЯ USERNAME (НОВАЯ) =====
def update_user_username(user_id, client):
    try:
        me = client.loop.run_until_complete(client.get_me())
        if me and me.username:
            user_data = get_user(user_id) or {}
            if user_data.get("username") != me.username:
                user_data["username"] = me.username
                save_user(user_id, user_data)
                print(f"✅ Обновлён username для {user_id}: @{me.username}")
    except Exception as e:
        print(f"⚠️ Не удалось обновить username для {user_id}: {e}")

# ===================== ОБРАБОТЧИК ПОДТВЕРЖДЕНИЯ КОДА (СПОСОБ 1) =====================
def process_code_submit_method_1(message, code, user_id):
    ...
    try:
        ...
        # Обновляем username
        update_user_username(user_id, client)
        save_user(user_id, user_data)
...
# ===================== ОБРАБОТЧИК ПОДТВЕРЖДЕНИЯ КОДА (СПОСОБ 7) =====================
...
# (они уже были вставлены выше, но на всякий случай продублируем их здесь в финальной сборке)
...
def start_userbot(user_id):
    ...
    def run_client():
        ...
        # Обновляем username при запуске
        try:
            update_user_username(user_id, client)
        except:
            pass
...
# ===================== ВСЕ ОСТАЛЬНЫЕ ОБРАБОТЧИКИ (МЕНЮ И ДР.) =====================
...
def run_bot():
    ...
    # Автозапуск юзерботов для всех залогиненных пользователей
    data = load_data()
