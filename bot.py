...
def show_main_menu(message):
    ...
    if not is_logged_in:
        ...
        markup.row(
            ...
            types.InlineKeyboardButton("🤖 ИИ", callback_data="ai_menu")
        )
        markup.row(
            types.InlineKeyboardButton("🔇 Чаты", callback_data="chats_menu"),
...
def process_phone_method_1(message):
    ...
    if not phone.startswith("+"):
        ...
        markup = types.InlineKeyboardMarkup()
        markup.row(
            types.InlineKeyboardButton("🔙 Шаг 3", callback_data="auth_back_to_api_hash"),
            types.InlineKeyboardButton("❌ Отменить вход", callback_data="cancel_auth")
        )
        msg = send_new_message(user_id, text, reply_markup=markup)
        bot.register_next_step_handler(msg, process_phone_method_1)
...
if __name__ == "__main__":
    ...
    # Запускаем автозапуск юзерботов в асинхронном цикле
    try:
